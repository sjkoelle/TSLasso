...
class FlassoManifold(FlassoExperiment):
    ...
    def get_ts_pb(self, df_M, dslow, tb_1):
        nsel = df_M.shape[0]
        dfast = df_M.shape[1]
        ts_pb = np.zeros((nsel, dfast,dslow))
        output = np.zeros((nsel, tb_1.shape[1], dslow))
        for i in range(nsel):
            ts_pb[i] = np.linalg.svd(df_M[i,:])[0][:,:dslow]
            output[i] = np.matmul(tb_1[i], ts_pb[i])
        return(output)
